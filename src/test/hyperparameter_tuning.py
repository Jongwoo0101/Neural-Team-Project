...
MODEL_HPARAMS = {
    ...
    #DropOut 비율, (use_dropout=True시 사용)
...
all_combinations = product(
    ...
    hidden_depths, l2_lambdas, use_bns, use_drops, drop_ratios
)
# 총 조합 수 계산 (결과 출력용)
total_combinations = len(lrs) * len(batch_sizes) * len(max_iters) * len(activation_inits) * \
                     len(hidden_depths) * len(l2_lambdas) * len(use_bns) * len(use_drops) * len(drop_ratios)

# 2. 조합 순회 시작
for lr, bs, max_i, act_init, depth, l2, use_bn, use_drop, drop_r in all_combinations:
    # 3. 조건부 로직을 사용하여 유효한 조합만 실행
    # DropOut을 사용하지 않는데 DropOut 비율이 0이 아닌 경우 스킵
    if use_drop == False:
        continue
    # DropOut을 사용하는데 DropOut 비율이 0인 경우 (정규화 목적상 의미 없음) 스킵
    # if use_drop == True and drop_r == 0:
    #     continue
    # 현재 실험 파라미터 딕셔너리 생성
    current_params = {
    ...
    try:
        ...
        network = MultiLayerNetExtend(
            ...
            use_dropout=current_params['use_dropout'],
